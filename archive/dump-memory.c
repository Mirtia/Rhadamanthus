...
static int bareflank_setup(vmi_init_data_t** init_data_ptr,
                           memory_map_t** memmap_ptr) {
  ...
  memory_map_t* memmap =
      malloc(sizeof(memory_map_t) + sizeof(addr_t) * 2 * e820_entries);
...
static void usage(const char* argv0) {
  ...
  printf(
      "  -k, --kvmi-socket     use the specified kvmi socket for KVM driver\n");
...
int main(int argc, char** argv) {
  ...
  vmi_mode_t mode;
  if (VMI_FAILURE == vmi_get_access_mode(NULL, domain_name, VMI_INIT_DOMAINNAME,
                                         init_data, &mode)) {
    goto free_setup_info;
  }
  ...
  if (VMI_FAILURE == vmi_init(&vmi, mode, (void*)domain_name,
                              VMI_INIT_DOMAINNAME, init_data, NULL)) {
