...
static bool init_vmi(int argc, char** argv) {
  ...
  if (VMI_FAILURE ==
      vmi_init_complete(&vmi, input, init, NULL, config_type, config, NULL)) {
...
int main(int argc, char** argv) {
  ...
  if (VMI_FAILURE == vmi_get_struct_member_offset_from_json(
                         vmi, json, "printk_log", "len", &length_offset))
    goto error_exit;

  addr_t text_len_offset;
  if (VMI_FAILURE == vmi_get_struct_member_offset_from_json(
                         vmi, json, "printk_log", "text_len", &text_len_offset))
    goto error_exit;

  addr_t time_stamp_offset;
  if (VMI_FAILURE ==
      vmi_get_struct_member_offset_from_json(vmi, json, "printk_log", "ts_nsec",
                                             &time_stamp_offset))
    goto error_exit;

  addr_t text_offset;
  if (VMI_FAILURE ==
      vmi_get_struct_size_from_json(vmi, json, "printk_log", &text_offset))
    goto error_exit;
  ...
  if (log_first_idx < log_next_idx) {
    ...
    if (VMI_FAILURE == vmi_read_va(vmi, log_buf_addr + log_first_idx, 0, length,
                                   log_buf, NULL))
      goto error_exit;
  } else {
    ...
    if (VMI_FAILURE == vmi_read_va(vmi, log_buf_addr + log_first_idx, 0,
                                   log_buf_2nd_half, log_buf, NULL))
      goto error_exit;

    if (VMI_FAILURE == vmi_read_va(vmi, log_buf_addr, 0, log_next_idx,
                                   log_buf + log_buf_2nd_half, NULL))
      goto error_exit;
  }
