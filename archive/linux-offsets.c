...
void show_usage(char* arg0) {
  ...
  fprintf(stderr,
          "    %s --domid 17 --json-kernel /opt/kernel.json --only-kpgd\n",
          arg0);
}
...
int main(int argc, char** argv) {
  ...
  if (VMI_FAILURE ==
      vmi_get_access_mode(vmi, domain, init_flags, NULL, &mode)) {
  ...
  if (only_output_kpgd) {
    ...
    printf(
        ...
        "kpgd:0x%lx\n",
        linux_tasks, linux_mm, linux_pid, linux_name, linux_pgd, linux_kaslr,
        linux_init_task, kpgd);
  }
