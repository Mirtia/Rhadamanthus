...
static bool setup_vmi(vmi_instance_t* vmi, char* domain, uint64_t domid,
                      char* json, char* kvmi, bool init_events,
                      bool init_paging) {
  printf(
      "Init vmi, init_events: %i init_paging %i domain %s domid %lu json %s "
      "kvmi %s\n",
  ...
  if (json) {
    if (VMI_OS_UNKNOWN ==
        (os = vmi_init_os(*vmi, VMI_CONFIG_JSON_PATH, json, NULL))) {
...
static void do_list_pages(addr_t pagetable) {
  ...
  while (loop) {
    ...
    printf("\t0x%lx [0x%x %s%c] -> 0x%lx\n", info->vaddr, info->size,
           READ_WRITE(entry) ? "rw" : "r-", NX(entry) ? '-' : 'x', info->paddr);
...
int main(int argc, char** argv) {
  ...
  if (!domid || (!read && !write && !list_pages) || (read && write) ||
      (!list_pages && (!limit || !filepath))) {
    usage();
    return -1;
  }
  ...
  ACCESS_CONTEXT(ctx, .translate_mechanism = VMI_TM_PROCESS_DTB,
                 .pt = target_pagetable, .addr = address);
  ...
  while (iters--) {
    ...
    if (read) {
      if (VMI_SUCCESS ==
          vmi_read(vmi, &ctx, access_length, (void*)&buffer, NULL))
