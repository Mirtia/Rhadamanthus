...
event_response_t execve_enter_cb(vmi_instance_t vmi, vmi_event_t *event) {
  ...
  if (event->interrupt_event.gla == virt_do_execve) {
    ...
    for (i = 0; i < num_sig; i++) {
      if (!strncmp(hash_val, sig_list[i], MD5_DIGEST_LENGTH)) {
        ...
        /**
         * We can change the API's parameter into invalid, via modifying RDI
         * registers.
         */
        /*
        ...
        */
        /**
         * or change the code path by modifying the RIP register, and return
         * values in RAX
         */
        ...
        /**
         * Invalid the return value. If the return value is a pointer, can
         * change to 0x0
         */
  ...
  if (VMI_FAILURE ==
      vmi_write_32_va(vmi, virt_do_execve, 0, &do_execve_orig_data)) {
...
int introspect_process_block(char *name) {
  ...
  /**
   * get the list of process signatures that to block their launch, from the
   * file blacklist.txt.
   */
  num_sig = 0;
  char _line[256];
   ...
   */
  if (VMI_FAILURE ==
      vmi_read_32_va(vmi, virt_do_execve, 0, &do_execve_orig_data)) {
