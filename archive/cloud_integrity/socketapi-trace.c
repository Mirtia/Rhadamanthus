 ...
 * For EPT, we need two vmi_event_t structure: accept_enter_event for accept
 * API; connect_enter_event for connect API For INT3, we only need to use
 * accept_enter_event to cover both accept and connect API We also use
 * accept_setp_event to denote the step event from accept or connect API
 */
...
/**
 * virtual and physical address for sys_accept, sys_connect, and return address
 * from sys_accept
 */
...
#else
/**
 * original instruction data of sys_accept, sys_connect or return instruction
 * from sys_accept
 */
...
event_response_t socket_enter_cb(vmi_instance_t vmi, vmi_event_t* event) {
   ...
   * This event should be interrupted at most once, to retrieve the return
   * address of inet_csk_accept
   */
  if (event_addr == virt_sys_accept) {
     ...
     * It turns out that the inet_stream_connect is on the same page with the
     * return address of inet_csk_accept so no need to set up and register a new
     * event Otherwise you need to set up an new event
     */
    ...
    if ((virt_leave_sys_accept >> 12) != (virt_sys_accept >> 12) &&
        (virt_leave_sys_accept >> 12) != (virt_sys_connect >> 12)) {
...
#else
    if (VMI_FAILURE == vmi_read_32_va(vmi, virt_leave_sys_accept, 0,
                                      &leave_sys_accept_orig_data)) {
   ...
   */
    ...
    /**
     * If this is called by the socket api, then the length of the parameter
     * should be 16.
     */
    if ((int)rdx == 16) {
      uint8_t ip_addr[4];
   ...
   */
    ...
    if (rax > 0) {
      ...
      uint32_t ip_addr;
      /**
       * socket struct offsets can be obtained by running findsocket in the
       * tools folder
       */
  ...
  if (event_addr == virt_sys_accept) {
    if (VMI_FAILURE ==
        vmi_write_32_va(vmi, virt_sys_accept, 0, &sys_accept_orig_data)) {
    ...
    if (VMI_FAILURE ==
        vmi_write_32_va(vmi, virt_sys_connect, 0, &sys_connect_orig_data)) {
      printf("failed to write memory.\n");
      vmi_destroy(vmi);
      exit(1);
    }
    ...
    if (VMI_FAILURE == vmi_write_32_va(vmi, virt_leave_sys_accept, 0,
                                       &leave_sys_accept_orig_data)) {
      printf("failed to write memory.\n");
      vmi_destroy(vmi);
      exit(1);
...
int introspect_socketapi_trace(char* name) {
   ...
   */
  if (VMI_FAILURE ==
      vmi_read_32_va(vmi, virt_sys_accept, 0, &sys_accept_orig_data)) {
    printf("failed to read the original data.\n");
    vmi_destroy(vmi);
    return -1;
  }
  ...
  if (VMI_FAILURE ==
      vmi_read_32_va(vmi, virt_sys_connect, 0, &sys_connect_orig_data)) {
    printf("failed to read the original data.\n");
    vmi_destroy(vmi);
    return -1;
  }
