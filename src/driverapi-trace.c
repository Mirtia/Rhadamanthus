 ...
 * For EPT, we need two vmi_event_t structure: module_enter_event for module
 * filesystem setup; chrdev_enter_event for device registration For INT3, we
 * only need to use module_enter_event to cover both events We also use
 * module_setp_event to denote the step event from either module filesystem
 * setup or device registration
 */
...
event_response_t driver_enter_cb(vmi_instance_t vmi, vmi_event_t *event) {
  ...
  if (event_addr == virt_mod_sysfs_setup) {
    ...
    /**
     * the module structure offset can be obtained by running findmodule in the
     * tools folder
     */
    ...
    printf("Process [%d] inserts a kernel module: %s Addr: 0x%" PRIx64
           " - 0x%" PRIx64 "\n",
   ...
   */
    ...
    /**
     * The following three lines of codes can bypass the register_chardev with
     * returning value as error.
     */
  ...
  if (event_addr == virt_mod_sysfs_setup) {
    if (VMI_FAILURE == vmi_write_32_va(vmi, virt_mod_sysfs_setup, 0,
                                       &mod_sysfs_setup_orig_data)) {
    ...
    if (VMI_FAILURE == vmi_write_32_va(vmi, virt_register_chrdev, 0,
                                       &register_chrdev_orig_data)) {
      printf("failed to write memory.\n");
      vmi_destroy(vmi);
      exit(1);
    }
...
int introspect_driverapi_trace(char *name) {
   ...
   */
  if (VMI_FAILURE == vmi_read_32_va(vmi, virt_register_chrdev, 0,
                                    &register_chrdev_orig_data)) {
  ...
  if (VMI_FAILURE == vmi_read_32_va(vmi, virt_mod_sysfs_setup, 0,
                                    &mod_sysfs_setup_orig_data)) {
    printf("failed to read the original data.\n");
    vmi_destroy(vmi);
    return -1;
