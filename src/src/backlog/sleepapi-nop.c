  ...
  do {
    ...
    if (pid1 == pid) {
      ...
      for (i = 0; i < num_proc; i++) {
        if (!strcmp(list[i],
                    vmi_read_str_va(vmi, current_process + name_offset, 0)))
          return 1;
      }
      break;
    }
    status_t status =
        vmi_read_addr_va(vmi, next_list_entry, 0, &next_list_entry);
...
event_response_t nanosleep_enter_cb(vmi_instance_t vmi, vmi_event_t *event) {
  ...
  if (event->interrupt_event.gla == virt_sys_nanosleep) {
    ...
    if (VMI_FAILURE ==
        vmi_write_32_va(vmi, virt_sys_nanosleep, 0, &sys_nanosleep_orig_data)) {
    ...
    event->interrupt_event.reinject = 0;
    if (VMI_FAILURE == vmi_write_32_va(vmi, virt_sys_clock_nanosleep, 0,
                                       &sys_clock_nanosleep_orig_data)) {
      printf("failed to write memory.\n");
      exit(1);
    }
    vmi_register_event(vmi, &clock_nanosleep_step_event);
...
int introspect_sleepapi_nop(char *name) {
  ...
  /**
   * get the list of process names that to NOP their sleep API, from the file
   * blacklist.txt.
   */
  num_proc = 0;
  char _line[256];
  ...
  /**
   * get the address of two syscalls for sleep: sys_nanosleep and
   * sys_clock_nanosleep
   */
  ...
  clock_nanosleep_enter_event.mem_event.physical_address =
      phys_sys_clock_nanosleep;
   ...
   */
  if (VMI_FAILURE ==
      vmi_read_32_va(vmi, virt_sys_nanosleep, 0, &sys_nanosleep_orig_data)) {
    printf("failed to read the original data.\n");
    vmi_destroy(vmi);
    return -1;
  }

  if (VMI_FAILURE == vmi_read_32_va(vmi, virt_sys_clock_nanosleep, 0,
                                    &sys_clock_nanosleep_orig_data)) {
    printf("failed to read the original data.\n");
    vmi_destroy(vmi);
    return -1;
