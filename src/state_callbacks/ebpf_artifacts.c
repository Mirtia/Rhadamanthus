...
static const char* known_rootkit_names[] = {
    "ebpfkit", "triplecross", "bpfdoor", "badbpf",
    "exechijack", "pidhide", "sudoadd", "textreplace",
    NULL
};

// Suspicious syscall hooks commonly used by rootkits
static const char* suspicious_syscalls[] = {
    "sys_getdents", "sys_getdents64", "sys_kill", "sys_openat",
    "sys_read", "sys_write", "sys_execve", "sys_ptrace",
    "sys_bpf", "sys_perf_event_open", "sys_timerfd_settime",
    NULL
};
 ...
 */
static int enumerate_ebpf_programs(vmi_instance_t vmi, ebpf_detection_ctx_t* ctx) {
    ...
    if (vmi_translate_ksym2v(vmi, "prog_idr", &prog_idr_addr) != VMI_SUCCESS) {
        ...
        if (vmi_translate_ksym2v(vmi, "prog_array", &prog_idr_addr) != VMI_SUCCESS) {
 ...
 */
static int enumerate_ebpf_maps(vmi_instance_t vmi, ebpf_detection_ctx_t* ctx) {
    addr_t map_idr_addr;
 ...
 */
static int check_hidden_programs(vmi_instance_t vmi, ebpf_detection_ctx_t* ctx) {
...
static void analyze_bpf_maps(ebpf_detection_ctx_t* ctx) {
    ...
    for (uint32_t i = 0; i < ctx->map_count; i++) {
        ...
        if (map->max_entries > 100000) {
            log_warn("Large BPF map detected: %s (%u entries)", 
                     map->name, map->max_entries);
...
static int check_syscall_table_integrity(vmi_instance_t vmi) {
    ...
    if (vmi_translate_ksym2v(vmi, "sys_call_table", &sys_call_table_addr) != VMI_SUCCESS) {
...
static void compare_with_previous_state(ebpf_detection_ctx_t* current) {
    ...
    if (current->prog_count > previous_ctx.prog_count) {
        log_warn("New eBPF programs detected: %u -> %u",
    ...
    if (current->map_count > previous_ctx.map_count) {
        log_warn("New eBPF maps detected: %u -> %u",
...
static void save_current_state(ebpf_detection_ctx_t* current) {
    ...
    if (current->prog_count > 0) {
        previous_ctx.programs = calloc(current->prog_count, sizeof(ebpf_prog_info_t));
