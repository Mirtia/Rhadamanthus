...
static bool get_legitimate_syscall_entry(vmi_instance_t vmi,
  ...
  for (int i = 0; syscall_symbols[i] != NULL; i++) {
    if (vmi_translate_ksym2v(vmi, syscall_symbols[i], syscall_entry) ==
        VMI_SUCCESS) {
      log_info("Found legitimate syscall entry: %s at 0x%" PRIx64,
               syscall_symbols[i], (uint64_t)*syscall_entry);
      return true;
    }
  }

  log_warn("Could not resolve any known syscall entry symbols.");
  return false;
}
...
static bool read_msr_lstar(vmi_instance_t vmi, unsigned int vcpu_id,
                           addr_t* lstar_value) {
  if (!lstar_value) {
    log_error("Output pointer is NULL in read_msr_lstar.");
    return false;
  }

  if (vmi_get_vcpureg(vmi, lstar_value, MSR_LSTAR, vcpu_id) != VMI_SUCCESS) {
    log_error("Failed to read MSR_LSTAR from vCPU %u.", vcpu_id);
    return false;
  }
...
uint32_t state_msr_registers_callback(vmi_instance_t vmi, void* context) {
  ...
  log_info("Reading MSR_LSTAR state from %u vCPU(s).", num_vcpus);
  ...
  for (unsigned int cpu = 0; cpu < num_vcpus; cpu++) {
    ...
    if (!read_msr_lstar(vmi, cpu, &lstar_value)) {
      log_warn("Failed to read MSR_LSTAR from vCPU %u.", cpu);
      continue;
    }

    log_info("vCPU %u MSR_LSTAR: 0x%" PRIx64, cpu, (uint64_t)lstar_value);
    ...
    if (!within_kernel_text) {
      log_info(
          "vCPU %u MSR_LSTAR points outside kernel text section (0x%" PRIx64
          " not in [0x%" PRIx64 ", 0x%" PRIx64 "])",
          cpu, (uint64_t)lstar_value, (uint64_t)kernel_start,
          (uint64_t)kernel_end);
    } else {
      log_info("vCPU %u MSR_LSTAR within kernel text bounds", cpu);
    }

    // Report if differs from expected syscall entry
    if (has_legitimate_ref && lstar_value != legitimate_syscall) {
      log_info(
          "vCPU %u MSR_LSTAR differs from expected syscall entry (0x%" PRIx64
          " vs 0x%" PRIx64 ")",
