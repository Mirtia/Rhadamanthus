...
static GPtrArray* load_known_files() {
  ...
  if (!file) {
    log_warn("Failed to open known files file: %s. Proceeding with empty list.", KNOWN_FILES_FILE);
    return known_files;
  }
...
uint32_t state_dir_string_matching_callback(vmi_instance_t vmi, void* context) {
  ...
  log_info("Known files loaded successfully. File system inspection not yet implemented.");
  log_info("This callback would detect files/directories that exist in kernel memory");
  log_info("but are hidden from userspace, indicating potential rootkit activity.");
