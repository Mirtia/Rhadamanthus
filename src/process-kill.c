...
event_response_t kill_step_cb(vmi_instance_t vmi, vmi_event_t *event) {
  ...
  if (kill_flag == 0) {
    ...
    reg_t sys_kill;
    vmi_translate_ksym2v(vmi, "sys_kill", &sys_kill);
    vmi_set_vcpureg(vmi, sys_kill, RIP, event->vcpu_id);
...
event_response_t kill_enter_cb(vmi_instance_t vmi, vmi_event_t *event) {
  ...
  if (event->interrupt_event.gla == virt_schedule) {
    kill_flag = 0;
    if (VMI_FAILURE ==
        vmi_write_32_va(vmi, virt_schedule, 0, &schedule_orig_data)) {
...
int introspect_process_kill(char *name, char *arg) {
  ...
  if (VMI_FAILURE ==
      vmi_read_32_va(vmi, virt_schedule, 0, &schedule_orig_data)) {
